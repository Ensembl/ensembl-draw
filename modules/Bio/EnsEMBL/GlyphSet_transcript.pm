...
sub _init {
  ...
  foreach my $gene ( @{$self->features()} ) { # For alternate splicing diagram only draw transcripts in gene
    ...
    foreach my $transcript (@{$gene->get_all_Transcripts()}) {
      ...
      my $coding_start = defined ( $transcript->coding_start() ) ? $transcript->coding_start :  $transcript->start();
      my $coding_end   = defined ( $transcript->coding_end() )   ? $transcript->coding_end :    $transcript->end();
