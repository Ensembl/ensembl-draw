package Bio::EnsEMBL::GlyphSet::annot_gene_label_lite;
###############################################################################
#   annot_gene_label_lite
#   GlyphSet for the Sanger Annotators website, based on Ensembl
#
#   This GlyphSet is a modified copy of gene_label_lite.pm, and is  used in the
#   Annotation site ContigView.
###############################################################################
use strict;
use vars qw(@ISA);
...
##############################################################################
    ...
    my $known_col      = $Config->get( 'annot_gene_label_lite' , 'known' );
    my $hi_col         = $Config->get( 'annot_gene_label_lite' , 'hi' );
    my $unknown_col    = $Config->get( 'annot_gene_label_lite' , 'unknown' );
    my $ext_col        = $Config->get( 'annot_gene_label_lite' , 'ext' );
    my $pseudo_col     = $Config->get( 'annot_gene_label_lite' , 'pseudo' );
    my $max_length     = $Config->get( 'annot_gene_label_lite' , 'threshold' ) || 2000000;
    my $navigation     = $Config->get( 'annot_gene_label_lite' , 'navigation' ) || 'off';
    my $max_length_nav = $Config->get( 'annot_gene_label_lite' , 'navigation_threshold' ) || 200000;
    my $im_width       = $Config->image_width();
    my $type           = $Config->get( 'annot_gene_label_lite' , 'src' );
    my $pix_per_bp     = $Config->transform->{'scalex'};
    my $fontname       = "Tiny";
    my ($font_w_bp,$h) = $Config->texthelper->px2bp($fontname);
    my $w              = $Config->texthelper->width($fontname);
    my $sanger_colours = { 
           'Novel_CDS'        => $Config->get('annot_gene_label_lite','sanger_Novel_CDS'), 
           'Putative'         => $Config->get('annot_gene_label_lite','sanger_Putative'), 
           'Known'            => $Config->get('annot_gene_label_lite','sanger_Known'), 
           'Novel_Transcript' => $Config->get('annot_gene_label_lite','sanger_Novel_Transcript'), 
           'Pseudogene'       => $Config->get('annot_gene_label_lite','sanger_Pseudogene'), 
    }; 
...
##############################################################################
    ...
    foreach my $g (@genes) {
        ...
        my $depth = $Config->get('annot_gene_label_lite', 'dep');
        if ($depth > 0){ # we bump
