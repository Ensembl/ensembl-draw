...
sub href {
  ...
  if ( $Container->{'hits'}{$hit}{'datalib'} !~ /pfamfrag/i && $hit !~ /^WP.+[A-Z]$/i ) {
	$alignment =
        "/$ENV{'ENSEMBL_SPECIES'}/alignview?transcript=".$Container->{'transcript'}{'ID'}."&exon=".@{$Container->{'hits'}{$hit}{'exon_ids'}}[$exon_count]."&sequence=".$hit."&seq_type=$seq_type&db=".
