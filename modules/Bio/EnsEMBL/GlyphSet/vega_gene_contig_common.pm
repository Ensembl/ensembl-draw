package Bio::EnsEMBL::GlyphSet::vega_gene_contig_common;
use strict;
use vars qw(@ISA);
...
use Bio::EnsEMBL::GlyphSet::vega_gene_contig_common;
...
my ($text, $species, $script) = @_;
 my $label = new Sanger::Graphics::Glyph::Text({
        'text'      => $text,
        'font'      => 'Small',
        'absolutey' => 1,
        'href'      => qq[javascript:X=window.open(\'/$species/helpview?se=1&kw=$script#gene_lite\',\'helpview\',\'height=400,width=500,left=100,screenX=100,top=100,screenY=100,resizable,scrollbars=yes\');X.focus();void(0)],

        'zmenu'     => {
			'caption'       => 'HELP',
            "01:Track information..."     =>
qq[javascript:X=window.open(\\\'/$species/helpview?se=1&kw=$script#gene_lite\\\',\\\'helpview\\\',\\\'height=400,width=500,left=100,screenX=100,top=100,screenY=100,resizable,scrollbars=yes\\\');X.focus();void(0)]
        }
...
sub _init {
    ...
    my $pred_col      = $Config->get('gene_lite','pred');
    ...
    #
    # Draw all of the havana Genes
    #
    my $F = 0;

    foreach my $g (@{$vc->get_all_Genes('havana', )} ) {
      $F++;
