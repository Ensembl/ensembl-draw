...
sub my_label { 
  ...
  my $key = '_my_label';
  if( ! exists( $self->{$key} ) ){
    my $misc_set_code = $self->my_config('FEATURES') || return undef;
    ...
    my $misc_set = $msa_adaptor->fetch_by_code( $misc_set_code ) || return undef;
...
sub features {
    my ($self) = @_;
    my $misc_set_code = $self->my_config('FEATURES') || return [];
    #  die( "The FEATURES key of UserConfig must be a misc_set_code" );
